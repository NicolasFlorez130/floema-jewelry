...
const Home = ({ charged }) => {
   const doc = useContext(HomeContext);
   const doc2 = useContext(ProductsContext);
   ...
   useEffect(() => {
      ...
      bgImages.forEach(img => {
         ...
         gsap.fromTo(
            img,
            { opacity: 0, scale: 1 + random },
            { duration: 1 * random, opacity: 1, scale: 1, delay: 0.5 + random, ease: '"power4.in"' }
         );
      });
      ...
      mm.add('(min-width: 1024px)', () => {
         ...
         gsap.to('.home .image1, .home .image3', { y: 50 + 'vw' });
         gsap.to('.home .image2', { y: 100 + 'vw' });
         // gsap.to('.home .image3', { y: 100 + 'vw' });
   ...
   return (
      <section className="home | bg-orange h-[100vh] overflow-hidden relative ">
         ...
         <div className="imagesContainerSm | w-full absolute bottom-0 z-0">
            {[1, 2].map(n => {
               return (
                  <div
                     key={n}
                     className={`imagesGrid | flex-none gap-4 grid grid-cols-2 px-4 py-2 w-full 
                  sm:grid-cols-3 | lg:grid-cols-5`}>
                     {images.map((img, i) => {
                        ...
                        return (
                           <img
                              src={img.image.url}
                              className={`image${imagesAux++} backgroundImage | aspect-[2/3] object-cover relative w-full`}
                              alt="Floema jewelry home image"
                              key={i}
                           />
                        );
                     })}
         ...
         <div className="flex justify-center items-end h-[100vh] w-full">
            <button
               onClick={buttonClick}
               className="relative font-['Suisse'] border-2 border-light bottom-0 mb-6 py-4 px-16 rounded-[50%] text-light w-max z-30
               lg:text-xs hover:bg-light hover:text-orange transition-[.2s]">
