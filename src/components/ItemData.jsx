...
const ItemData = ({ item }) => {
   ...
   function switchImage() {
      ...
      tl.to(image.current, {
         duration: .5, rotateY: 90, ease: 'power3.in', onComplete: () => {
            image.current.src = image.current.src === item.product_image.url ? item.model_image.url : item.product_image.url;
         }
      })
      tl.to(image.current, { duration: .5, rotateY: 0, ease: 'power3.out' })
   }
   ...
   return (
      <section ref={container} className=" itemData | font-light text-light">
         <div ref={wrapper} className="wrapper | h-[100vh] py-14 px-6 w-full
            lg:p-14">
            <div className="firstContainer | m-auto max-w-[1200px] lg:grid lg:grid-cols-2">
               <div className="imageWrapper | pb-8 relative
               lg:inline-block lg:relative lg:w-full">
                  <img ref={image} onClick={switchImage} className="image | aspect-[3/4] m-auto object-cover opacity-0 w-[100%]
               sm:w-3/5 | lg:w-4/5" src={item.product_image.url} alt={item.alt} />
               ...
               lg:pl-8">
                  <div className="title | relative">
                     ...
                     <h2 className="font-['Suisse'] pb-6 text-lg uppercase
                        lg:text-sm">{item.collection}</h2>
            ...
            <div className="m-auto max-w-[1200px]">
               ...
               <div className="buttons | pt-8 relative">
                  ...
                  <button onClick={buttonClick} className="block border-[1px] border-light border-solid m-auto mt-12 px-12 py-6 rounded-[100%]
                  lg:mt-4">CLOSE</button>
               </div>
            </div>
         </div>
      </section >
